...
module.exports = {
    init: function *(info,socket,io,cb) {
        ...
        if(decode){
            let user = yield User.findOne({username: decode.user}).populate('online').populate('rooms');
            // v2.2 修改上线方式
            if(user.online){
                console.log('forcedOffline: ',user.username);
    ...
    getActive: function *(token,cb){
        ...
        if(decode){
            let user = yield User.findOne({username:decode.user});
            if(user){
                let lastOnlineTime = user.lastOnlineTime;
                let userList = yield Private.find({timestamp: {$gt: lastOnlineTime},to: user.username},null,{sort:'-timestamp'}).populate('from');
                let roomList = yield User.findOne({username: user.username}).populate({path: 'rooms',match: {lastMessage: {$gt: lastOnlineTime}}});
    ...
    leave: function *(socket,io) {
        let online = yield Online.findOne({socket:socket.id}).populate('user','username');
        if(online){
            console.log(online.user.username,'离线');
            User.updateInfo({
                username: online.user.username,
                onlineState: 'offline',
                lastOnlineTime: new Date().getTime()
            });
            io.emit('user leaved',{username:online.user.username});
        }
    ...
    reconnect: function *(token,cb) {
        let decode = jwt.verify(token,JWT_KEY);
        if(decode){
            let user = yield User.findOne({username: decode.user}).populate('online');
            if(user.online){
                ...
                if(userSave){
                    console.log('[reconnect]user leave:',user.username);
                    return cb({isSuccess: true});
                } else{
                    console.log('[reconnect]user leave error:',user.username);
