...
module.exports = {
    saveMessage: function *(message,socket,cb) {
        ...
        let user = yield User.findOneUser({username:message.username});
        if(user){
    ...
    savePrivateMessage: function *(message,socket,cb) {
        ...
        if(message.room === config.ROBOT_NAME){
            ...
            let toUser = yield User.findOne({username: message.room}).populate('online');
            let fromUser = yield User.findOne({username: message.username});
    ...
    getRichTextContent: function *(info,cb){
        let owner = yield User.findOne({username:info.owner});
        if(owner){
