...
module.exports = {
    createRoom: function *(socket,info,cb) {
        let user = yield User.findOne({username: info.user});
    ...
    getRoomInfo: function *(roomName,cb){
        ...
        if(room){
            cb({
                name: room.name,
                info: room.info,
                creater: room.creater.username,
                time: moment(room.createTime).format('lll'),
                avatar: room.avatar
            })
        } else{
            cb({
    ...
    joinRoom: function *(socket,info,cb) {
        let user = yield User.findOne({username: info.username});
        let room = yield Room.findOne({name: info.roomName}) || [];
    ...
    updateRoomInfo: function *(roomInfo,cb){
        ...
        if(decode.user){
            ...
            if(room){
                if(room.creater.username === decode.user){
    ...
    getRoomList: function *(token,cb){
        ...
        if(decode){
            let user = yield User.findOne({username: decode.user}).populate('rooms');
            if(user){
    ...
    searchRoom: function *(key,cb){
        let list = yield Room.find({'name': new RegExp(key,'i')},null,{limit:20});//正则查询
        if(list){
    ...
    getRoomActiveInfo: function *(roomName,cb){
        ...
        if(room && online){
            ...
            cb({
                ...
                info: room.info,
                name: room.name,
                creater: room.creater ? room.creater.username : ''
            })
        }
    }
}