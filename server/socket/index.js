...
module.exports = function (io) {
    io.on('connect',function (socket) {
        ...
        socket.on('login',(info,cb) => {
            console.log(info.username + 'login');
            co(user.verifyUser(info.username,info.password,cb)).catch((err) =>{
                callbackError(cb,err);
            });
        })
        socket.on('signUp',(info,cb) => {
            co(user.createUser(info.username,info.password,cb)).catch((err) =>{
                callbackError(cb,err);
            });
        })
        ...
        socket.on('getUserInfo',(username,cb) => {
            co(user.getUserInfo(username,cb)).catch((err) =>{
                callbackError(cb,err);
            });
        });
