...
function mapStateToProps(state) {
    ...
    let title = state.getIn([mode,'username']) || state.getIn([mode,'name']),
        creater = state.getIn([mode,'username']) || state.getIn([mode,'creater']),
        time = state.getIn([mode,'time']),
        avatar = state.getIn([mode,'avatar']),
        user = state.get('userState');
    let canChangeInfo = creater === state.getIn(['userState','username']);
