...
const handleReadLocalSetting = (username) => {
...
const handleInit = (info) => {
    ...
    getActiveList(info.token)(store.dispatch).then((res)=>{
        ...
        handleReadLocalSetting(resault.username);
...
socket.on('privateMessage', (message) => {
    ...
    if(state.setting.shield.indexOf(message.username) === -1){
        const audio = document.getElementById('audio1'),
              audioSpecial = document.getElementById('audio3'),
              avatar = message.avatar;
        message.room = message.username;
        ...
        if(document.hidden){
            ...
            if(state.setting.audioNotification){
                state.setting.special.indexOf(message.username) === -1 ? audio.play() : audioSpecial.play();
            }    
            state.setting.h5Notification ? notification.showNotification(message.username,{
...
socket.on('newMessage', (message) => {
    ...
    if(state.setting.shield.indexOf(message.username) === -1){
        const audio = document.getElementById('audio1'),
              audioSpecial = document.getElementById('audio3'),
              avatar = message.avatar,
              reg = new RegExp('@'+state.userState.username,'g');
        ...
        if(document.hidden){
            favico.addBage();
            if(state.setting.audioNotification){
                state.setting.special.indexOf(message.username) !== -1 || reg.test(message.content) ? audioSpecial.play() : audio.play() ;
            }
            state.setting.h5Notification && !(message.username === state.userState.username) ? notification.showNotification(message.username,{
                body:  message.type === 'pluginMessage' || message.username === config.PLUGIN_ROBOT ? '[plugin]' : message.content,
                icon: message.avatar,
            }) : null;
        } else if(!(message.username === state.userState.username) && state.setting.audioNotification && state.userState.curRoom !== message.room){
            state.setting.special.indexOf(message.username) !== -1 || reg.test(message.content) ? audioSpecial.play() : audio.play();
        }
    }
});
...
//         console.log('connect: ',state.userState.username);
// })
