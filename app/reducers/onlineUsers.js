...
export default function onlineUsers(state = defaultState,action) {
    switch (action.type) {
        ...
        case ADD_ONLINEUSER_INFO: {
            let user = state.get(action.user.username);
            if(user){
                return state.setIn([user.get('username'),'isOnline'],1);
            } else{
                let newUser = Immutable.fromJS(Object.assign({},action.user,{isOnline:1}));
                return state.set(action.user.username,newUser);
            }
        }
        ...
        case CHANGE_USER_INFO: {
            return state.setIn([action.user.username,'avatar'],action.user.avatar);
        }
        default: {
            return state;
        }
