        ...
        if(roomInfo.isPrivate){
            ...
            if(messages.length < 15){
                ...
                getPrivateHistory({
                    ...
                    toUser: state.userState.username,
    ...
    return (dispatch,getState) => {
        return new Promise((resolve) => {
            if(!info) {
                ...
                let toUser = state.userState.username;
...
export const getUserInfo = (username) => {
    return (dispatch,getState) => {
        const state = getState().toJS();
        dispatch(setSnackbarState({
            content: '获取'+username+'信息中',
            autoHideDuration: 1000,
            open: true
        }));
        return new Promise((resolve)=>{
            socket.emit('getUserInfo',username, (body) => {
                if(body.isError){
                    dispatch(setSnackbarState({
                        content: body.errMsg,
                        open: true
...
export const storageSetting = () => {
    return (dispatch,getState) => {
        ...
        if( typeof localStorage === 'object'){
            ...
            localStorage.setItem(state.userState.username,storage);
        }
    }
}
...
export const signUp = (username,password)=>{
    return new Promise((resolve,reject) => {
        socket.emit('signUp',{username,password},(info)=>{
            resolve(info);
        })
    })
}
